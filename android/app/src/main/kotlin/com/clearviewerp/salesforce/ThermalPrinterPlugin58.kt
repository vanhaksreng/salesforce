...
// Configuration
// ====================================================================
object PrinterConfig58 {
    const val DEFAULT_PRINTER_WIDTH = 384 // 58mm
...
// Data Classes
// ====================================================================
data class MonochromeData58(val width: Int, val height: Int, val data: ByteArray) {
    override fun equals(other: Any?): Boolean {
        if (this === other) return true
        if (javaClass != other?.javaClass) return false
        other as MonochromeData58
...
enum class ImageAlignment58(val value: Int) {
    LEFT(0),
    CENTER(1),
    RIGHT(2);
...
private var printerModel: PrinterModel58 = PrinterModel58.UNKNOWN


enum class PrinterModel58 {
    UNKNOWN,
...
// Main Plugin Class
// ====================================================================
class ThermalPrinterPlugin58 : FlutterPlugin, MethodChannel.MethodCallHandler {
    ...
    // Connection state
    @Volatile
    private var currentConnectionType = ConnectionType58.NONE
    private var printerWidth = PrinterConfig58.DEFAULT_PRINTER_WIDTH
        ...
        scope.launch(Dispatchers.IO) {
            try {
                ...
                val status = when (currentConnectionType) {
                    ConnectionType58.BLUETOOTH_CLASSIC -> {
                        try {
...
// ====================================================================
    ...
    private fun cleanupAllConnections() {
        try {
            ...
            currentConnectionType = ConnectionType58.NONE
        ...
        scope.launch(Dispatchers.IO) {
            try {
                ...
                if (connected) {
                    ...
                    // ============================================================
                    currentConnectionType = ConnectionType58.BLUETOOTH_CLASSIC
        ...
        try {
            ...
            mainHandler.postDelayed({
                if (pendingResults.remove(resultKey) != null) {
                    ...
                    result.error("TIMEOUT", "Connection timeout after ${PrinterConfig58.CONNECTION_TIMEOUT}ms", null)
                    bluetoothGatt?.disconnect()
                    bluetoothGatt?.close()
                    bluetoothGatt = null
                }
            }, PrinterConfig58.CONNECTION_TIMEOUT)
    ...
    private fun writeDataSmooth(data: ByteArray) {
        ...
        try {
            when (currentConnectionType) {
                ...
                ConnectionType58.BLUETOOTH_BLE -> {
...
// ====================================================================
    private fun printText(
        ...
        scope.launch {
            printMutex.withLock {
                try {
                    // ✅ KEY FIX: For 58mm printer, convert all small text to images
    ...
    // ✅ Detect if this is a separator line
    ...
    // ✅ FIXED: Size with proper Font B for small text
    val sizeCommand: Byte = if (isSeparatorLine) {
        0x00.toByte()  // Normal size for separators
    } else {
        when {
            fontSize > 30 -> 0x30.toByte()  // 4x size
            fontSize > 24 -> 0x11.toByte()  // 2x height
            fontSize >= 18 -> 0x00.toByte() // Normal Font A
            fontSize >= 12 -> 0x01.toByte() // Font B (smaller) ← FIX!
            else -> 0x01.toByte()           // Font B for very small
        }
    }
    ...
    private fun printRowUsingTextMethodBatched(columns: List<PosColumn58>, fontSize: Int) {
        // ✅ IMPROVED: Better character calculations for different font sizes
        val totalChars = when {
            fontSize >= 30 -> 20
            fontSize >= 24 -> 28
            fontSize >= 20 -> 32
        ...
        // ✅ IMPROVED: Better size command for small fonts
        val sizeCommand: Byte = when {
            fontSize >= 30 -> 0x30.toByte()  // 4x size
            fontSize >= 24 -> 0x11.toByte()  // 2x height
            fontSize >= 18 -> 0x00.toByte()  // Normal Font A
            else -> 0x01.toByte()            // Font B (smaller)
        }
        commands.addAll(listOf(ESC, 0x21, sizeCommand))
...
// ====================================================================
        ...
        return object : BluetoothGattCallback() {
            ...
            override fun onServicesDiscovered(gatt: BluetoothGatt, status: Int) {
                ...
                if (characteristic != null) {
                    ...
                    currentConnectionType = ConnectionType58.BLUETOOTH_BLE
    ...
    private fun connectNetwork(ipAddress: String, port: Int, result: MethodChannel.Result) {
        scope.launch(Dispatchers.IO) {
            try {
                ...
                currentConnectionType = ConnectionType58.NETWORK
    ...
    private fun renderTextToData(
        ...
        try {
            ...
            // ✅ Better scaling for small fonts on 58mm printer
            val baseFontSize = 20f
            val scaledFontSize = when {
                fontSize >= 30 -> baseFontSize * 2.0f   // 40px
                fontSize >= 24 -> baseFontSize * 1.5f   // 30px
                fontSize >= 18 -> baseFontSize * 1.2f   // 24px
                fontSize >= 14 -> baseFontSize * 0.9f   // 18px
                fontSize >= 12 -> baseFontSize * 0.75f  // 15px
                else -> baseFontSize * 0.6f             // 12px
            }

            println("📐 Font rendering: fontSize=$fontSize → scaledFontSize=$scaledFontSize")
            ...
            val padding = 1f  // ✅ REDUCED: Minimal padding
            ...
            // ✅ FIXED: Tighter line spacing
            val lineSpacingMultiplier = when {
                fontSize < 14 -> 0.95f  // Very tight for small text
                fontSize < 18 -> 0.92f  // Tight for medium text
                else -> 0.90f           // Normal spacing
            }
    ...
    private fun renderRowToData(columns: List<PosColumn58>, fontSize: Int): ByteArray? {
        var bitmap: Bitmap? = null
        try {
            // ✅ Better scaling for small fonts on 58mm printer
            val baseFontSize = 24f
            val scaledFontSize = when {
                fontSize >= 30 -> baseFontSize * 2.0f   // 48px
                fontSize >= 24 -> baseFontSize * 1.5f   // 36px
                fontSize >= 20 -> baseFontSize * 1.2f   // 28.8px
                fontSize >= 16 -> baseFontSize * 0.9f   // 21.6px
                fontSize >= 14 -> baseFontSize * 0.75f  // 18px
                fontSize >= 12 -> baseFontSize * 0.65f  // 15.6px
                fontSize >= 10 -> baseFontSize * 0.55f  // 13.2px
                else -> baseFontSize * 0.5f             // 12px
            }

            println("📐 Row rendering: fontSize=$fontSize → scaledFontSize=$scaledFontSize")

            val maxWidth = printerWidth.toFloat()
            val columnWidths = columns.map { (maxWidth * it.width) / 12 }

            // ✅ Calculate characters per column based on font size
            val totalChars = when {
                fontSize >= 30 -> 20
                fontSize >= 24 -> 28
                fontSize >= 20 -> 32
            ...
            // ✅ FIXED: Much tighter line spacing for compact look
            val lineSpacingMultiplier = when {
                fontSize < 14 -> 0.95f  // Very tight for small text
                fontSize < 18 -> 0.92f  // Tight for medium text
                else -> 0.90f           // Normal spacing
            ...
            // ✅ FIXED: Minimal padding for compact look
            val verticalPadding = when {
                fontSize < 14 -> 1f   // Minimal padding for small fonts
                fontSize < 18 -> 2f   // Small padding for medium fonts
                else -> 3f            // Normal padding
            }
            ...
            for (i in columns.indices) {
                ...
                for (lineIndex in lines.indices) {
                    ...
                    val x = when (column.align.lowercase()) {
                        ...
                        "right" -> currentX + colWidth - 2f  // Small right padding
        ...
        scope.launch {
            printMutex.withLock {
                ...
                try {
                    ...
                    val alignment = ImageAlignment58.fromInt(align)
                    scaledBitmap = resizeImage(bitmap, width)
                    val monochromeData58 = convertToMonochromeFast(scaledBitmap)

                    if (monochromeData58 == null) {
                        withContext(Dispatchers.Main) {
                    ...
                    val widthBytes = (monochromeData58.width + 7) / 8
                    commands.add((widthBytes and 0xFF).toByte())
                    commands.add(((widthBytes shr 8) and 0xFF).toByte())
                    commands.add((monochromeData58.height and 0xFF).toByte())
                    commands.add(((monochromeData58.height shr 8) and 0xFF).toByte())

                    commands.addAll(monochromeData58.data.toList())
        ...
        scope.launch {
            printMutex.withLock {
                ...
                try {
                    ...
                    val monochromeData58 = if (alignment != ImageAlignment58.LEFT) {
    ...
    private fun addPaddingToMonochrome(
        data: MonochromeData58,
        alignment: ImageAlignment58,
        paperWidth: Int
    ): MonochromeData58 {
        ...
        val leftPadding = when (alignment) {
            ImageAlignment58.LEFT -> 0
            ImageAlignment58.CENTER -> paddingTotal / 2
            ImageAlignment58.RIGHT -> paddingTotal
        }
        ...
        return MonochromeData58(paperWidth, data.height, newData)
    }


    private fun convertToMonochromeFast(bitmap: Bitmap): MonochromeData58? {
        try {
            ...
            return MonochromeData58(width, height, data)
...
//    }
    ...
    private fun getStatus(result: MethodChannel.Result) {
        ...
        val isConnected = when (currentConnectionType) {
            ConnectionType58.BLUETOOTH_CLASSIC -> bluetoothSocket?.isConnected ?: false
            ConnectionType58.BLUETOOTH_BLE -> bluetoothGatt != null && writeCharacteristic != null
            ConnectionType58.NETWORK -> networkSocket?.isConnected ?: false
            else -> false
        }
