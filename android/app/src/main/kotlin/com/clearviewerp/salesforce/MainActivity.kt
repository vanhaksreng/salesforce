...
class MainActivity : FlutterActivity() {
    ...
    override fun configureFlutterEngine(flutterEngine: FlutterEngine) {
        ...
        // Set up the method channel for thermal printer plugin
        ...
        printerPlugin = BluetoothPrinterPlugin(this, this)
        MethodChannel(flutterEngine.dartExecutor.binaryMessenger, printPosMethodChannel).setMethodCallHandler(printerPlugin)
                ...
                .setMethodCallHandler { call, result ->
                    try {
                        when (call.method) {
                            ...
                            "requestPermissions" -> {
                                val mode = call.argument<String>("mode") ?: "foreground"
                                permissionResultCallback = { granted -> result.success(granted) }
                                requestPermissions(mode)
                            }
                            "checkPermissions" -> {
                        ...
                        result.error(
                                "METHOD_ERROR",
                                "Failed to handle ${call.method}: ${e.message}",
                                null
                        )
                    }
                }
        ...
        try {
            ...
            if (hasPermission) {
                ...
                try {
                    ...
                    // Log.w("MainActivity", "Failed to send permissionChanged (Flutter engine
                    // likely detached): ${e.message}")
                }
    ...
    private fun requestLocationPermissions(mode: String) {
        try {
            ...
            try {
                ...
                // Log.w("MainActivity", "Failed to send error (Flutter engine likely detached):
                // ${e.message}")
            }
    ...
    private fun safeInvokeFlutterMethod(method: String, arguments: Any?) {
        try {
            ...
            android.util.Log.w(
                    "MainActivity",
                    "Failed to invoke Flutter method $method: ${e.message}"
            )
        }
    }

    private fun setupHtmlToPdfChannel(flutterEngine: FlutterEngine) {
        MethodChannel(flutterEngine.dartExecutor.binaryMessenger, htmlPdfChannel)
                .setMethodCallHandler { call, result ->
