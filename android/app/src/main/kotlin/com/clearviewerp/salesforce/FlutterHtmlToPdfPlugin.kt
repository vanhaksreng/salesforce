...
class FlutterHtmlToPdfPlugin : FlutterPlugin, MethodCallHandler {
    ...
    private fun convertHtmlToPdf(call: MethodCall, result: MethodChannel.Result) {
        ...
        HtmlToPdfConverter().convert(htmlFilePath.toString(), applicationContext, object : HtmlToPdfConverter.Callback {
            override fun onSuccess(filePath: String) {
                result.success(filePath)
            }

            override fun onFailure() {
                result.error("ERROR", "Unable to convert HTML to PDF", null)
            }
        })
    }
}
