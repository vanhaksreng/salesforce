...
class CustomersCubit extends Cubit<CustomersState> with DownloadMixin, AppMixin, MessageMixin {
  ...
  Future<void> getCustomers({Map<String, dynamic>? params, int page = 1}) async {
    try {
  ...
  Future<bool> createNewCustomer(String customerNo) async {
    final result = await _repos.storeNewCustomer(param: {"customer_no": customerNo});
    return result.fold(
      (l) {
  ...
  Future<void> getLatLng() async {
    try {
      ...
      emit(state.copyWith(latLng: LatLng(currentLocation.latitude, currentLocation.longitude)));
