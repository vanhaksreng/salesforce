...
class _ItemScreenState extends State<ItemScreen> with AutomaticKeepAliveClientMixin, MessageMixin {
  final _cubit = ItemsCubit();

  final _scrollController = ScrollController();
  final ValueNotifier<ShapeType> shapeType = ValueNotifier<ShapeType>(ShapeType.list);
  ...
  void _getSaleLines() {
    _cubit.getSaleLines(documentType: widget.documentType, scheduleId: widget.schedule.id);
  }
  ...
  bool _checkShowFilter() {
    return filterString.isNotEmpty && filterString != "IN {}" || statusStock.isNotEmpty ? true : false;
  }
  ...
  void _navigateToGroupScreenFilter() {
    Navigator.pushNamed(
      ...
      arguments: GroupFilterArgs(groupCodes: _selectedGroups, status: statusStock),
  ...
  void _navigateToProcessForm(Item item) {
    if (widget.documentType != kSaleCreditMemo && Helpers.toDouble(item.inventory) <= 0) {
    ...
    Navigator.pushNamed(
      ...
      arguments: SaleFormArg(item: item, schedule: widget.schedule, documentType: widget.documentType),
    ).then((value) {
      _getSaleLines();
        ...
        return Column(
          ...
          children: [
            SearchWidget(
              ...
              suffixIcon: Padding(
                padding: EdgeInsets.symmetric(vertical: 3.scale, horizontal: 3.scale),
                child: SizedBox(
                  width: 110.scale,
                  child: Row(
                    spacing: 4.scale,
                    ...
                    children: [
                      BtnIconCircleWidget(
                        ...
                        icons: Icon(
                          shapeType.value == ShapeType.grid ? Icons.view_list_rounded : Icons.grid_view_rounded,
                          color: mainColor50,
                          size: 20.scale,
                        ),
                      ...
                      BtnIconCircleWidget(
                        ...
                        icons: Icon(Icons.filter_list, color: mainColor50, size: 20.scale),
                        bgColor: grey20,
                        rounded: 4.scale,
                      ),
                    ],
  ...
  Widget _buildItem(List<Item> items, ShapeType valueShapeType, List<PosSalesLine> saleLines) {
    if (items.isEmpty) {
  ...
  Widget filterBtn({required VoidCallback onTap}) {
    return Badge(
      smallSize: 10,
      isLabelVisible: filterString.isNotEmpty && filterString != "IN {}" ? true : false,
      backgroundColor: red,
      child: BtnWidget(
        onPressed: onTap,
        icon: SvgWidget(assetName: kFilter, colorSvg: primary, padding: EdgeInsets.all(scaleFontSize(appSpace8))),
        height: 35.scale,
        width: 35.scale,
