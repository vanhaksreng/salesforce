...
class _SaleFormScreenState extends State<SaleFormScreen> {
  ...
  void _initializeControllers(List<SaleFormInput> inputs) {
    ...
    for (final input in inputs) {
      String qty = Helpers.formatNumber(input.quantity, option: FormatType.quantity, display: false);
    ...
    _disPercentageCntr.text = Helpers.formatNumber(_cubit.state.discountPercentage, option: FormatType.quantity);

    _disAmountCntr.text = Helpers.formatNumber(_cubit.state.discountAmt, option: FormatType.quantity);

    _manualPriceCntr.text = Helpers.formatNumber(_cubit.state.manualPrice, option: FormatType.price, display: false);
  }
      ...
      children: [
        ...
        BoxWidget(
          ...
          child: Row(
            ...
            children: [
              ImageBoxCoverWidget(
                image: ImageNetWorkWidget(imageUrl: _item.picture ?? "", width: 60.scale, height: 60.scale),
              ),
              Helpers.gapW(6),
              Expanded(
                child: Column(
        ...
        if (state.isExistedStd) ...[
          SizedBox(height: scaleFontSize(20)),
          const TextWidget(text: "discount_option_standard_sales", color: primary, fontWeight: FontWeight.bold),
              ...
              children: [
                ...
                buildTextFormFieldWidget(
                  ...
                  suffix: SizedBox(
                    ...
                    child: Align(
                      alignment: Alignment.center,
                      child: TextWidget(text: Helpers.currencySymble(), fontSize: 18, color: primary),
                    ),
                  ),
                ),
              ],
        ...
        if (state.isExistedStd && state.canModifyPrice) ...[
          SizedBox(height: scaleFontSize(20)),
          const TextWidget(text: "manual_selling_price_standard_sales", color: primary, fontWeight: FontWeight.bold),
          SizedBox(height: scaleFontSize(2)),
          const TextWidget(text: "this_will_overwrite_item_default_selling_price", fontSize: 12, color: orangeColor),
          SizedBox(height: scaleFontSize(10)),
          BoxWidget(
            padding: EdgeInsets.all(scaleFontSize(15)),
            child: Row(
  ...
  Widget buildTextFormFieldWidget({
    ...
    return Expanded(
      child: TextFormFieldWidget(
        ...
        onChanged: onChanged,
        inputFormatters: isNumber ? [const QuantityInputFormatter(decimalRange: 8)] : null,
        keyboardType: isNumber ? const TextInputType.numberWithOptions(decimal: true, signed: true) : keyboardType,
      ),
    );
  }
}
