...
class ProcessCubit extends Cubit<ProcessState>
    with AppMixin, DownloadMixin, PermissionMixin {
  ...
  Future<void> getCountPosmAndMerchandising(
    ...
    await _taskRepo
        .getSalesPersonScheduleMerchandises(
          param: {'status': kStatusOpen, 'visit_no': state.schedule?.id},
        )
        .then((resonse) {
  ...
  Future<void> getCountCompetitorPromotion(SalespersonSchedule schedule) async {
    await _taskRepo
        .getCompetitorItemLedgetEntry(
          param: {'status': kStatusOpen, 'schedule_id': schedule.id},
        )
        .then((resonse) {
          resonse.fold((l) => throw GeneralException(l.message), (r) {
            emit(
  ...
  Future<void> getCountCollection(SalespersonSchedule schedule) async {
    await _taskRepo
        .getCashReceiptJournal(
          param: {'status': kStatusOpen, 'source_no': schedule.id},
        )
        .then((resonse) {
          resonse.fold((l) => throw GeneralException(l.message), (r) {
            emit(
              state.copyWith(
                countCollection: r == null ? 0 : 1,
                isLoading: false,
              ),
            );
          });
  ...
  Future<void> getCountCheckStock(SalespersonSchedule schedule) async {
    ...
    await _taskRepo
        ...
        .then((resonse) {
          resonse.fold((l) => throw GeneralException(l.message), (r) {
            emit(state.copyWith(isLoading: false, countCheckStock: r.length));
          });
        });
  }
  ...
  Future<void> getCountSales(SalespersonSchedule schedule) async {
    await _taskRepo
        .getPosSaleHeaders(
          params: {'source_no': schedule.id, 'source_type': kSourceTypeVisit},
        )
        .then((resonse) {
  ...
  Future<void> getItemUom({
    required String itemNo,
    required String uOmCode,
  }) async {
    try {
      emit(state.copyWith(isLoading: true));
      final response = await _taskRepo.getItemUom(
  ...
  Future<void> getItemPromotionHeaders() async {
    try {
      ...
      await _taskRepo.getItemPromotionHeaders().then((response) {
        response.fold((l) => throw GeneralException(l.message), (
          promotionHeaders,
        ) {
          emit(state.copyWith(isLoading: false));
        });
